        ...
        drivebase.setDefaultCommand(new DefaultDriveCommand(
            ...
            () -> {
               // double inX = -driver.getLeftY(); // swap intended
                double inX = -driver.getY(); // swap intended
               // double inY = -driver.getLeftX();
                double inY = -driver.getX();
            ...
            //() -> modifyAxis(-driver.getRightX()) * Drivebase.MAX_ANGULAR_VELOCITY_RADIANS_PER_SECOND));
            () -> modifyAxis(-operatorJoystick.getRightX()) * Drivebase.MAX_ANGULAR_VELOCITY_RADIANS_PER_SECOND));
// Part of vision - Brodie Wozniak
...
//        List<WPI_TalonFX> falcons = systems.getDrivebase().getMotors();
        List<CANSparkMax> neos = systems.getDrivebase().getMotors();
...
//                SmartDashboard.putNumber(falconNames[i] + " Temp", falcons.get(i).getTemperature());
                SmartDashboard.putNumber(neoNames[i] + " Temp", neos.get(i).getMotorTemperature());                
            }
...
// Part of Vision - Brodie Wozniak
    ...
    private void configureBindings() {
        ...
        operatorJoystick.back().onTrue(runOnce(drivebase::zeroGyroscope));

        // D-Pad cardinal directions
        // tried to change the numbers to make bot slower and it didn't work 
        ...
        BooleanSupplier isManualAdjustment = () -> {
           // return modifyAxis(-driver.getRightX()) != 0;
            return modifyAxis(-operatorJoystick.getRightX()) != 0; 
        };
     ...
     /* driver.a().onTrue(new DriveLockedRotCommand(systems, defaultDrive, 180, isManualAdjustment));
        ...
        driver.button(9).onTrue(new DriveLockedRotCommand(systems, defaultDrive, 180, isManualAdjustment));
        driver.button(10).onTrue(new DriveLockedRotCommand(systems, defaultDrive, 270, isManualAdjustment));
        driver.button(8).onTrue(new DriveLockedRotCommand(systems, defaultDrive, 90, isManualAdjustment));
        driver.button(6).onTrue(new DriveLockedRotCommand(systems, defaultDrive, 0, isManualAdjustment));
       ...
       // driver.leftTrigger().onTrue(new ArmGoalGroup( // Stow
        driver.button(2).onTrue(new ArmGoalGroup( // Stow
            systems,
            Constants.armStow,
       ...
       // driver.rightTrigger().onTrue(new ArmGoalGroup(
        driver.button(2).onTrue(new ArmGoalGroup(
            systems,
